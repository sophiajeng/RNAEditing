...
workflow reditools2_workflow {
    String rna_dna_fastq_file
    #String dna_fastq_file
    #String samples_file

    #where reditools_rna_dna_fastqs.txt contains fastq files in the form sample1_rna_r1.fastq.gz sample2_rna_r2.fastq.gz sample1_dna_r1.fastq.gz sample2_dna_r2.fastq.gz(one row per sample)
    #where reditools_dna_fastqs.txt contains fastq files in the form sample1_r1.fastq.gz sample2_r2.fastq.gz (one row per sample)
    Array[Array[File]] rna_dna_fastqs = read_tsv(rna_dna_fastq_file)
    #Array[Array[File]] dna_fastqs = read_tsv(dna_fastq_file)


    #Array[String] input_samples = read_lines(samples_file)

    #need to makre sure sample is in the same order for both rna and dna
    #Array[Pair[String, Array[File]]] sample_rna_fastqs = zip(input_samples, rna_fastqs)
    #Array[Pair[String, Array[File]]] sample_dna_fastqs = zip(input_samples, dna_fastqs)
    #Array[Pair[String,Array[File]]] sample_rna_dna_fastqs = zip(input_samples,rna_dna_fastqs)

    String star_index_dir = "/home/exacloud/gscratch/mcweeney_lab/jengs/genomes/STAR_idx/STAR_GRCm39"    
    String bwa_index_dir = "/home/exacloud/gscratch/mcweeney_lab/jengs/genomes/GRCm39"
    File ref_genome_dir = "/home/exacloud/gscratch/mcweeney_lab/jengs/genomes/GRCm39/GRCm39.primary_assembly.genome.fa"
    File ref_genome_gtf = "/home/exacloud/gscratch/mcweeney_lab/jengs/genomes/Strand_detection/mm39.ncbiRefSeq_sorted.gtf"
